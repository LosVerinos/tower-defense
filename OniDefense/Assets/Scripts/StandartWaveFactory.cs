...
namespace Game
{
    public class StandardWaveFactory : WaveFactory
    {
        public override Wave CreateWave(int waveNumber)
        {
            ...
            if (zombie3Ratio > 0) builder.AddZombieType(new HeavyZombie().index, zombie3Ratio);
            builder.AddZombieType(new ClassicZombie().index, zombie1Ratio);
            if (zombie2Ratio > 0) builder.AddZombieType(new FastZombie().index, zombie2Ratio);
            if (zombie4Ratio > 0) builder.AddZombieType(new CrowZombie().index, zombie4Ratio);
